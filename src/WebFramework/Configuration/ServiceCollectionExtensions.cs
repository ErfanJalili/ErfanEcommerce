...
namespace WebFramework.Configuration
{
    public static class ServiceCollectionExtensions
    {
        ...
        public static void AddPermissinBaseAuthorization(this IServiceCollection services)
        {

            var permissionDefinitionContext = new PermissionDefinitionContext();
            var permissionsDefinitionProvider = new PermissionsDefinitionProvider();
            permissionsDefinitionProvider.Define(permissionDefinitionContext);


            services.AddAuthorization(c =>
            {
                foreach (var permissionGroup in permissionDefinitionContext.PermissionGroups)
                {
                    var permissions = permissionGroup.Permissions;
                    foreach (var permission in permissions)
                    {
                        c.AddPolicy(permission.Name, p =>
                        {
                            p.RequireClaim(CustomClaimTypes.Permission, permission.Name);
                        });
                        foreach (var innerPermission in permission.Permissions)
                        {
                            c.AddPolicy(innerPermission.Name, p =>
                            {
                                p.RequireClaim(CustomClaimTypes.Permission, innerPermission.Name);
                            });
                        }
                    }
                }
